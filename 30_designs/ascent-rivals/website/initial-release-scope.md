...
Ship one greenfield Next.js/React website that becomes the primary destination for both game discovery and player activity. The release replaces the current marketing site and Ascentun for all approved non-blockchain behavior, then improves the public pilot, course, and team statistics experience.
...
- Ascentun's approved non-blockchain workflows;
- better pilot, course, and team statistics using both exact tables and purposeful visualizations;
- canonical routes, redirects, SEO metadata, cutover, rollback, and legacy retirement for replaced behavior.
...
|---|---|---|
| Discovery | Home search across gauntlets, teams, pilots, and admin-visible sponsors | Preserve grouped public search for gauntlets, teams, pilots, and courses; keep sponsor results administrative and out of public/general creator search |
...
| Sponsor operations | Admin list/detail, create, edit, colors, social links, and media | Required administrative parity with no public or general creator registry; creators retain only a scoped existing-sponsor picker inside gauntlet authoring |
| Authentication | Steam login/callback, token refresh, session-aware permissions, logout | Required at launch |
| Media | Purpose lookup, signed upload URLs, previews, metadata, and entity media attachment | Required launch parity for team, gauntlet, and sponsor workflows; use generic labeled media controls except for the direct gauntlet billboard path |
...
- every approved non-blockchain Ascentun workflow has a tested Website V2 destination or an explicitly approved retirement;
- all required marketing routes and content have migrated with canonical redirects;
...
- Steam auth, permission checks, uploads, empty/error states, responsive layouts, accessibility, SEO, observability, cutover, and rollback are verified.

## Confirmed Operational Scope

All non-blockchain operational workflows move to Website V2 at launch:

- team creation and management;
- gauntlet creation, editing, and deletion;
...
- retain existing sponsor records, sponsor media, gauntlet–sponsor relationships, and relationship tiers;
