...
- [[eventun-medals-progression-goals-challenges-rewards-solution-design]]
...
- Eventun commit `34b42861f2a698be50b0d7de134881544d072658`;
- Ascentun commit `deca8522428c586d0cfd030f2b2e843f87fe657f`;
...
- Match delivery remains at most once in the current client. Stable batch ids, event ids, producer sequence, source classification, artifact association, and idempotent acceptance are implemented in the coordinated Eventun/game-client working trees; the narrow fact layer remains under rework before this design is re-approved.
...
1. The implemented foundation adds identified complete-match batches, one source-tagged `game_event` relation, stable event ids and sequence, and physical source/event-type partitions. Legacy `server_event` and `client_event` reads remain only until F14/F15 cutover and conversion. Automatic sender retry remains a separate behavior change.
2. Implemented synchronous derivation produces narrow match, heat, match-player, heat-player, and progression-contribution facts. Detailed lap/checkpoint telemetry remains in raw partitions; qualification serving inputs remain F14 work.
...
The sub-16 ms objective applies to measured hot database operations, not end-to-end calls through AccelByte or the public service. See [[../../10_research/ascent-rivals/eventun-team-postgresql-derivation-review]].

## Delivery Sequence

### Phase 0: Correctness Foundations

Foundation work through F12 is implemented in the working trees:
...
- Accountun and Cardanoun proof-of-concept adapters are isolated, while TapTools and token-gating dependencies are removed;
...
- move progression workers to idempotent fact contributions and harden queue claiming, retries, and dead-letter behavior in F13;
- add incremental serving projections and cut over lifetime/public reads in F14;
- convert retained legacy telemetry and remove legacy event dependencies in F15;
...
Complete the planned foundation task list before re-approving this design. Do not expand the reset into an ORM, migration runner, CI service, generic repository layer, or unrelated product rewrite.
