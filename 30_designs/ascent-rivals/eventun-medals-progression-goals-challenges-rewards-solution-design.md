...
- `HeatStart` carries a game-authored canonical/special-case signal used by progression and record policies.
...
- authoritative heat canonical/special-case context
...
| Field | Purpose |
| --- | --- |
| `code` | Stable metric code such as `medal.count`. |
...
| `scope_id` | `career`, `daily:YYYY-MM-DD`, `weekly:YYYY-Www`, `season:<code>`, etc. |
...
| Field | Purpose |
| --- | --- |
| `code` | Pool code such as `daily_default`. |
...
| Field | Purpose |
| --- | --- |
| `code` | Stable reward bundle code. |
...
| `reward_type` | `accelbyte_item`, `accelbyte_currency`, `accelbyte_season_pass_xp`, future `title`, or `custom`. |
...
- Store season/pass context and XP quantity for AccelByte Season Pass XP rewards.
...
| --- | --- |
| AccelByte item | SKU |
| AccelByte currency | Currency code |
| AccelByte Season Pass XP | Season/pass context plus XP quantity |
...
- Use metadata fields for Eventun bundle id, completion id, goal code, goal version, source kind, and source match context when available.
- Use a stable transaction id based on the reward bundle, for example `eventun-reward:<bundle_id>`.

### AccelByte Season Pass XP Rewards
...
- Use `reward_type='accelbyte_season_pass_xp'`.
...
- After XP is granted, AccelByte owns the resulting season pass XP, tier progression, and tier reward claim state.
...
Current AccelByte docs show Season Pass XP grant as an amount-based server operation and do not show a caller-provided transaction id comparable to Platform Fulfillment V2. Eventun must therefore use its local reward bundle and grant-attempt ledger to prevent duplicate XP grants before invoking AccelByte. On uncertain timeout, Eventun should reconcile against AccelByte player season state or require operator review before retrying if duplicate XP would be harmful.
...
POST /v1/admin/progression/goals/{code}/versions
POST /v1/admin/progression/goals/{code}/versions/{version}/activate
...
3. Confirm whether `canonical` alone is enough in `HeatStart`, or whether `rulesetKey` and `rulesetVersion` should be required from the start.
4. Confirm whether public APIs should expose active challenge assignment/progress for other players in V1 or keep that surface self-only despite low sensitivity.
