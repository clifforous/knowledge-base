...
Repository restructuring and governance are implemented. The manual-use cases in the
[pass 3 governance ledger](../repository-restructure/pass-3-governance-ledger.md) should be
exercised before implementation of the `kb` tool. Current repository placement, metadata,
and task-log rules are authoritative in [ORGANIZATION.md](../../../ORGANIZATION.md); this
document defines the intended multi-contributor tooling direction.
...
The current manual path, filename, metadata, entry types, and disposition vocabulary are
defined in [ORGANIZATION.md](../../../ORGANIZATION.md#task-owned-decision-and-delta-logs).
The future `kb` implementation should preserve that contract unless manual validation leads
to a reviewed policy change.
...
- already represented
- keep pending with a concrete reason
- informational only
- rejected
- superseded
...
The personal repository now has current-system, initiative, decision, source, archive, and
transient-work boundaries; root governance; human/AI reading indexes; and an exercised
