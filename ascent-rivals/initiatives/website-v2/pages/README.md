# Website V2 Page Specifications

Status: in-progress
Status detail: Page specifications range from approved product boundaries to draft visual
and contract details; read each status line.

## Public And Player Pages

- [Homepage](homepage.md)
- [Gauntlets index](gauntlets-index.md)
- [Gauntlet detail](gauntlet-detail.md)
- [Player directory](player-directory.md)
- [Player profile](player-profile.md)
- [Course leaderboards](course-leaderboards.md)
- [Teams index](teams-index.md)
- [Team profile](team-profile.md)

These specifications describe Website V2 intent. They do not override the
[current website](../../../system/website.md) or current Eventun system chapters.

Website V2 has no sponsor index or detail page. The active
[sponsor-administration handoff](../sponsor-administration-handoff.md) assigns registry and
media operations to the Eventun Extend App while retaining bounded gauntlet-context display.
