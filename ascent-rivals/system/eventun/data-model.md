...
```mermaid
flowchart LR
    producer["Client or dedicated server<br/>complete match batch"] --> ingest["Authenticate, classify source,<br/>validate and canonicalize"]

    subgraph transaction["One accepted-batch transaction"]
        raw["Batch ledger, event identities<br/>and partitioned raw events"] --> facts["Current match, heat, player<br/>and progression facts"]
        facts --> projections["Record, career, seasonal<br/>and gauntlet serving projections"]
        projections --> queue["Fact-backed progression jobs"]
    end

    ingest --> raw
    projections --> reads["Fresh bounded product reads"]
    raw --> detail["Bounded raw-detail reads"]
    queue -.->|scheduled after commit| goals["Counters, goals, completions<br/>and reward bundles"]
    goals -.->|automatic fulfillment| rewards["AccelByte rewards"]

    artifact["Independent replay-artifact operation"] -.-> reads
    repair["Targeted repair or full rebuild"] -.-> facts
    repair -.-> projections
```

The raw inserts, fact derivation, serving projection updates, and progression-job
enqueue are atomic. Scheduled goal evaluation and reward fulfillment begin only
after commit. Replay artifacts are accepted independently. The canonical flow,
including collision handling, exact storage relations, rollback, optional
post-ingest integrations, and repair/rebuild entry points, is maintained in the
[[identified-match-ingestion#End-to-end accepted batch flow|identified-match-ingestion contract]].
...
Accepted batches synchronously maintain idempotent serving state in the same transaction. `match_serving_projection_state` records one projected batch's source receipt, exact fact fingerprint, projection schema version, projector version, and timestamps. Accepted matches and repairs lock affected players in UUID order before additive singleton rollup updates, so concurrent distinct matches for one player serialize without losing either contribution. `player_course_record_contribution` retains each eligible batch/heat/player/category source identity, time, loadout value, occurrence, session/match, course, and `single_player_mode`; `player_course_record` retains the deterministic current winner keyed by source, record category, course, and player. Record categories remain server/client `finish` and `lap` plus client-only `high_cost_finish`, `high_cost_lap`, `low_cost_finish`, and `low_cost_lap`, with the legacy inclusive `<= 10,000` and `<= 3,000` loadout-value thresholds. Winner and top-N order is time, loadout value ascending with null last, then deterministic identity tie-breaks; `player_rank` applies the same two-non-null loadout ordering. Mutable profile and team presentation are joined at read time rather than stored as record identity.

`player_career_contribution` is one canonical dedicated-server contribution per batch/player. `player_career_rollup` and `player_course_career_rollup` retain mergeable sums, counts, and minima for matches, kills, deaths, crashes, obelisks, podiums, play/lap/finish time, weight, circuit points, and credits; APIs derive averages and null behavior at read time. Only explicit `reported_podium_finish = true` contributes a podium. Match History starts from source/player-selective `match_player_fact` rows, performs keyed `match_fact` and `match_artifact` lookup, orders by MatchStart time, and associates SessionStart client version and replay data without scanning global fact history. SessionStart and PlayerJoin provenance use the complete time/batch/sequence/event tuple boundary, so an equal-time event ordered after MatchStart cannot change the frozen version or bot policy. Time-trial self-history selects and limits eligible current fact identities first and then fetches exact bounded client PlayerHeatStart/End detail, including the reported best-lap field and loadout. Time-trial and post-match candidate joins explicitly constrain every fact relation by source and are supported by the placed-player completion-history index; measured full shapes may instead choose a selective match-first primary-key join. Full match summary and current-match insight baselines/metrics remain deliberately session/match-scoped raw reads for bot rows, loadouts, complete heat/standing shape, and metrics absent from compact facts. Detailed post-match self-history analogously selects current canonical server Ascent/placed candidates from the fact graph, applies the ascension cutoff and per-heat limit, and then fetches only those selected PlayerHeatStart/End raw identities.

Eventun owns the implemented logical season model. A season has an immutable UUID identity, mutable non-unique title, regular/off-season kind, and finite UTC half-open window. Windows cannot overlap, may touch, and may leave gaps. Server match facts resolve nullable season attribution from MatchStart. Client facts receive a season only when MatchStart and trusted batch receipt resolve to the same non-null season; otherwise they remain unseasoned while contributing to lifetime behavior. This is season-attribution eligibility, not anti-cheat or match-result validation. Raw events and match-player facts do not duplicate season identity, producers never submit it, and deterministic historical conversion deliberately leaves converted facts null-season.

`player_season_course_record` contains the seasonal best-lap and best-finish winner keyed by season, source, category, course, and player. Its winner identity is a foreign key to `player_course_record_contribution`, and its season must equal the nullable season joined from the winner batch's `match_fact`; no contribution season column is stored. Winner and tie ordering is exactly the lifetime record ordering. Lifetime and seasonal leaderboard reads assemble course/category groups set-wise and evaluate mutable `player_view` presentation once per request rather than once per group. Normal projection, targeted repair, fact repair, and full rebuild assert that parent attribution and seasonal winners agree. Normal derivation and fact repair hold the shared schedule lock; targeted serving-only repair and full serving rebuild consume persisted attribution without taking it. Match/player serving projections use schema/projector generation `2/2`; gauntlet projection and sealed qualification evidence remain `1/1`.

Uncovered matches remain unseasoned and continue to support lifetime statistics and unfiltered Match History. Match History returns only an optional season UUID; clients resolve current mutable title and kind from the catalog. The initial seasonal projections cover best lap, best finish, and their ranks while existing lifetime records and career aggregates remain. Season splitting and reassignment between attributed seasons are deferred recovery concerns; telemetry defects use fact/contribution repair. Seasons do not define PostgreSQL partitions, retention tiers, game builds, AccelByte Season Pass identity, or MMR ownership. The implemented `game_event` hierarchy preserves client/server source and event-type pruning because that layout has demonstrated query-performance value. See [[ascent-rivals/initiatives/eventun-foundation/eventun-telemetry-lifecycle-plan|eventun-telemetry-lifecycle-plan]].
...
Current schema delivery deliberately uses `a*` through `d*` as the canonical clean-slate schema and a manually applied `migration.sql` production delta without a permanent migration runner or ledger. Canonical data/configuration is sequential: `d0_data_media_purpose.sql`, `d1_data_progression.sql`, and `d2_insight_policy.sql`. Synchronous accepted-transaction projections replaced `d3_schedule_refresh_views.sql`, `refresh_leaderboard_materialized_views`, all eight leaderboard and four gauntlet materialized views, and operational pg_cron provisioning. The former frozen production delta was deployed and removed on 2026-07-13; `migration/migration.sql` now targets the current deployed production baseline and is the stable filename for the one pending one-time delta. The pending offline cutover converts retained history, reconciles replacement outputs while both representations exist, records successful acceptance, and only then permits destructive legacy cleanup. Its temporary conversion and rehearsal machinery is removed after the production cutover is accepted. Optional development fixtures remain independently ordered as `t0_seed_courses.sql` through `t3_seed_teams.sql`, so production-delta cleanup never renumbers them. The PostgreSQL image runs canonical `a*` through `d*` files; no production delta or development fixture participates in automatic empty-database initialization.
