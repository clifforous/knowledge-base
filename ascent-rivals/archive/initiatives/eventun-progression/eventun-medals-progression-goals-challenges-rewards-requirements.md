...
Archive notice: Historical requirements incorporated into the current progression model.
Use `ascent-rivals/system/eventun/progression.md` for current behavior.
...
Post-V1 review items are collected in `ascent-rivals/initiatives/eventun-progression/eventun-progression-next-phase-ideation-notes.md`.
...
Battle Pass XP is a first-class V1 reward entry type. It is distinct from item and currency rewards because AccelByte Season Pass XP is granted through the Season Pass service rather than normal Platform catalog fulfillment. Battle Pass XP reward entries should store the XP amount as `quantity` and should not require `item_sku`, `item_id`, `currency_code`, or a catalog target. Eventun should use its configured AccelByte namespace for Season Pass calls.

Battle Pass XP should apply to the current published AccelByte Season Pass at fulfillment time by default. If no current season exists, the reward should use an explicit fallback policy, such as awarding nothing for that entry or granting ARC instead. If no fallback is configured, fulfillment should fail clearly and remain retryable after Season Pass setup is corrected. Tying Battle Pass XP to a specific season is a supported future option, but it is not required for V1 because time-windowed seasonal challenges already constrain when their goals can be completed. This reward type is different from regular account or game XP; regular XP should be modeled later as a statistics/account-progression reward type, not as Season Pass XP.
...
3. Reward bundles must support ARC, skins, and Battle Pass XP as expected V1 reward examples, and should remain extensible to future reward types such as titles.
...
40. Battle Pass XP rewards should grant to the current published AccelByte Season Pass by default and should support an explicit no-current-season fallback, such as no reward for that entry or ARC replacement.
41. Dedicated administration UI is deferred until repeated workflows justify dedicated tools.

## Open Requirements Questions

No open V1 product requirements are currently identified. Post-V1 product ideas and review questions are tracked in `ascent-rivals/initiatives/eventun-progression/eventun-progression-next-phase-ideation-notes.md`.
