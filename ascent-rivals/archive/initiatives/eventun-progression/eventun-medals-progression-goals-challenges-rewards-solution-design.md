...
Archive notice: Historical implemented and future-facing design. Use
`ascent-rivals/system/eventun/progression.md` for current behavior and the active progression
initiative for unfinished authoring or product work.
...
Requirements: `ascent-rivals/initiatives/eventun-progression/eventun-medals-progression-goals-challenges-rewards-requirements.md`
Draft/publish implementation plan: `ascent-rivals/initiatives/eventun-progression/eventun-progression-draft-publish-implementation-plan.md`
...
| `operator_key` | Optional generated human-readable reference for logs, CSV readability, and advanced operator lookup. UI label: `Code` when shown. It is not the durable identity and should not be required as manual data entry. |
| `goal_type` | `achievement` or `challenge`. |
| `title`, `description` | Default English player-facing copy for the next publish. |
...
| `title`, `description`, `presentation`, `visibility` | Copied default English player-facing data. |
...
Admin tools and imports should be UUID-first. When importing, a blank `id` / `goal_id` means create a new draft definition; a populated `id` / `goal_id` means update the draft definition. `operator_key` exists for generated labels, CSV readability, logs, and advanced lookup. It should be optional and generated when omitted. The admin UI may label it as `Code`, but it should not be the primary identity or a required authoring field. Backend APIs and CSV may keep the existing `operator_key` field name until the API is renamed.

Operator search should not depend on `operator_key`. Goal authoring APIs or UI projections should support searching by title, tag/category, kind, mastery flag, reward type, reward target, requirement metric, and extracted requirement dimensions such as medal name, weapon SKU, part SKU, course code, and method. A client-side fuzzy search over all paged goal summaries is acceptable for the initial admin UI while goal counts remain modest. If this becomes too slow, add server-side search or extracted search indexes without changing the source-of-truth requirement JSON model.

### Goal Text Localization

V1 localization is limited to goal `title` and `description`. Requirement expressions, metric codes, medal codes, SKUs, category values, visibility values, and ids are not localized through the goal system.

The default `title` and `description` stored on `goal_definition` and copied to `published_goal` are English fallback copy. Optional localized variants should be stored separately from gameplay rules so translation fixes do not require a new gameplay publish. A simple target shape is:

`goal_definition_localization`

| Field | Purpose |
| --- | --- |
| `goal_id` | Draft goal being localized. |
| `locale` | BCP 47 locale tag such as `en-US`, `fr`, or `fr-CA`. |
| `title`, `description` | Localized draft copy for the locale. |
| `updated_at` | Last translation edit time. |

`published_goal_localization`

| Field | Purpose |
| --- | --- |
| `published_goal_id` | Published goal snapshot being localized. |
| `locale` | BCP 47 locale tag. |
| `title`, `description` | Localized published copy for the locale. |
| `updated_at` | Last translation edit time. |

Publishing a goal should copy available draft localization rows to the published snapshot. Translation corrections on `published_goal_localization` are presentation-only changes and may be patched with audit after publish without changing requirement, reward, challenge assignment, or completion semantics.

Client-facing goal reads should accept an optional requested locale. The response should resolve `title` and `description` by requested locale, then language root, then English/default published copy. Missing translations must not fail the request. APIs may expose a resolved-locale debug field later, but the normal game-client path should receive display-ready `title` and `description`.
...
The admin UI should treat direct rewards as the default authoring path. An operator can add one or more rewards directly while creating an achievement, mastery, or challenge. Eventun still stores those direct rewards as generated single-goal reward bundle definitions behind the scenes. Reusable reward packages remain available for repeated packages, but they should not be required for the common single-reward case.

Operator-facing terminology should reserve "Reusable reward package" for explicitly reusable `bundle_kind=reusable` rows. Generated single-goal reward bundles are storage artifacts for direct rewards and should not be presented to operators as bundles.
...
- Reusable reward packages are an authoring convenience.
- Direct rewards may still be stored as generated draft reward bundles behind the scenes.
...
- Draft reward bundles do not need `active` or `inactive` status in the target model. Use `archived_at` to hide obsolete reusable reward packages from normal authoring.
...
```text
GET /v1/progression/goals?locale=en-US
GET /v1/player/{player_id}/medals
GET /v1/player/{player_id}/progression?locale=en-US
```
...
```text
GET  /v1/player/me/progression?locale=en-US
...
GET  /v1/player/me/matches/{session_id}/{match_id}/progression-summary?locale=en-US
```

`GET /v1/player/me/challenges/active` should ensure current daily, weekly, and monthly assignments exist before returning.

The active-challenges response should return assigned challenge records, not just bare goal rows. The game client needs assignment id/status, scope, period key, period start/end, progress values, locale-resolved goal title/description, goal metadata, and reward preview data to render the main challenge preview and current-period challenge route. Current-period completed assignments should remain visible until the period expires.

Reward preview labels are hints, not the only player-facing reward naming path. For AccelByte catalog-backed items, the game client should prefer the localized AccelByte catalog or client catalog display name resolved by SKU. For ARC and Battle Pass XP, the client can use localized fixed UI labels and the reward quantity. Eventun should still include reward type, quantity, SKU or currency where applicable, and an English/default label fallback for debugging and unsupported clients.
...
GET  /v1/admin/progression/localizations/export
POST /v1/admin/progression/localizations/imports
GET  /v1/admin/progression/localizations/imports/{import_id}
POST /v1/admin/progression/localizations/imports/{import_id}/apply
...
Goal localization should use a separate export/import workflow from the main goal definition CSV. The main goal CSV should remain focused on goal identity, requirement rules, rewards, and optional pool membership staging. Localization CSV should export one row per goal and locale with default English copy for translator context plus localized title/description fields. Import should upsert or delete draft/source localization rows and then sync affected published localization rows so text-only translation fixes can become visible without changing requirement or reward snapshots.

Goal import must support round-trip edit workflows:

- A populated `goal_id` updates that existing draft goal.
- A blank `goal_id` creates a new draft goal.
- `code` / `operator_key` is optional and should be generated when omitted.
- Updating a row with its own existing code must not produce a duplicate-code warning.
- On create, a copied or conflicting code should not be treated as identity. Preview may generate a unique replacement code and show the generated value, or return a clear row-level blocker if automatic replacement is not implemented.
- Code conflicts only matter when a row explicitly tries to set a code owned by a different goal and the system is not going to generate a replacement.
- Export should include enough data for AI-assisted and spreadsheet-assisted editing, including `goal_id`, code, tag/category, requirement JSON, reward mode, direct reward entries, and relevant challenge-pool membership columns.
- Generated single-goal reward bundles are implementation detail. Exported rows that use direct rewards must include reward entry columns and should use a `reward_mode=inline` indicator. Import apply should create or replace the direct reward from the row data rather than requiring operators to preserve generated reward bundle ids.
- CSV round-trip editing must support modifying direct rewards. When an existing row has `goal_id` and `reward_mode=inline`, changes to the `reward_N_*` columns should replace that goal's generated direct reward during import apply.
- Reusable reward package rows should use `reward_mode=bundle` and `reward_bundle_definition_id`.
- No-reward rows should use `reward_mode=none` or omit reward columns in a way the preview explicitly labels as no reward.
- Battle Pass XP reward rows should be represented as `reward_type=battlepass_xp`, quantity as XP amount, and no item SKU, item id, or currency code.
- Goal import may stage challenge-pool membership edits for challenge goals, such as adding a newly created or updated challenge to one or more draft pools. Import apply must not publish those pools automatically.

The admin UI should support bulk selection and draft-safe bulk operations for content setup. At minimum this includes bulk tag changes, bulk archive, and draft-only delete where every selected goal is safe to delete. Published snapshots and player history remain immutable; when a goal has published/runtime history, archive is the safe operator action rather than hard delete.

The admin UI should fetch normalized reward targets from Eventun, not AccelByte directly, and should hide generated single-goal reward bundles behind a direct reward editor.
...
- Goal tags should be editable, filterable, and included in CSV import/export. The current backend category field may continue to store the value until a separate tag model exists.
...
- Keep reusable reward packages as authoring convenience only; published goal snapshots should copy exact reward details.
...
- Requirements draft: `ascent-rivals/initiatives/eventun-progression/eventun-medals-progression-goals-challenges-rewards-requirements.md`
